...
(function() {
    ...
    async function initApp() {
        ...
        renderApp();
        bindEvents();
        if (window.tagsUI?.renderTags) window.tagsUI.renderTags();
    }

    // ===== CARGAR TAREAS =====
    async function loadTasks() {
        try {
            tasks = await window.tasksAPI.getTasks();
        } catch (error) {
    ...
    function renderTaskList(filteredTasks) {
        ...
        container.querySelectorAll('[data-action="toggle-task"]').forEach(btn => {
            btn.addEventListener('click', async (e) => {
                ...
                try {
                    await window.tasksAPI.toggleTaskComplete(taskId);
                    await loadTasks();
                    renderApp();
                } catch (error) {
    ...
    function openTaskDetail(taskId) {
        ...
        (async () => {
            try {
                ...
                overlay.querySelector('#modalTaskTitle').textContent = task.title;
                overlay.querySelector('#modalTaskDesc').textContent = task.description || 'Sin descripción';
                overlay.querySelector('#modalToggleComplete').textContent = task.completed ? 'Marcar pendiente' : 'Marcar completada';

                const metaContainer = overlay.querySelector('#modalMetaTags');
                let metaHtml = '';
                if (task.priority) metaHtml += `<span class="task-status badge-${task.priority}">${task.priority}</span>`;
                if (task.dueDate) metaHtml += `<span class="task-due">📅 ${formatDate(task.dueDate)}</span>`;
                ...
                if (subContainer && window.renderSubtasks) {
                    window.renderSubtasks(taskId, subContainer);
                }
                ...
                overlay.querySelector('#modalDeleteTask').addEventListener('click', async () => {
                    if (confirm('¿Eliminar esta tarea definitivamente?')) {
                        await window.tasksAPI.deleteTask(taskId);
                        await loadTasks();
                        overlay.remove();
                        renderApp();
                    }
    ...
    function bindEvents() {
        ...
        // --- MENÚ DE USUARIO (¡AQUÍ ESTABA EL ERROR!) ---
        ...
        // --- Configuración y Notificaciones (ejemplo) ---
    ...
    window.getCurrentTagFilter = () => selectedTag;
